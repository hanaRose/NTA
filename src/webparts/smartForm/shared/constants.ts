...
export type TenderPhase = 1 | 2 | 3;

// Helper to infer phase number from a value (string/number)
export function parsePhase(v: any): TenderPhase | null {
  if (v === null || v === undefined) return null;
  const s = String(v).trim().toLowerCase();
  if (s === '1' || s.indexOf('phase 1') > -1 || s.indexOf('pase 1') > -1) return 1;
  if (s === '2' || s.indexOf('phase 2') > -1 || s.indexOf('pase 2') > -1) return 2;
  if (s === '3' || s.indexOf('phase 3') > -1 || s.indexOf('pase 3') > -1) return 3;
  return null;
}
